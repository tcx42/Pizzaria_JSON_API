...
describe('POST /orders', ()=>{
    test('should add new order', async()=>{
    try {
        const res = await request(api).post('/api/orders').send({table_number: 3, items: [{pizza: 'Bufala', quantity: 1}]})
...
describe('GET /orders/:id', () => {
    ...
    test('Should respond with 404 case invalid id', async () => {
        const res = await request(api).get('/api/orders/700')
